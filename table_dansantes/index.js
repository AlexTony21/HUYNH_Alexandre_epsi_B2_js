...
function drawTable() {
  ...
  for (i=0; i<modulo; i++){
    let x = CentreX + (radius * Math.cos(i*step));
    let y = CentreY + (radius * Math.sin(i*step));
    let x1 = CentreX + (radius *Math.cos((i*table)*step));
    let y1 = CentreY + (radius *Math.sin((i*table)*step));
